...
if __name__ == '__main__':
    ...
    def plot_first_last_hv(nsga, name, errorbars=True, hv_no_agent=[]):
        """ Plot the hypervolume for the first and last agent target network """
        hv = nsga.hv_dict
        first_hv = np.array([np.array(hv[idx]) for idx in range(1, int(nsga.agent.replace/nsga.NGEN) +1)])
        final_hv = np.array([np.array(hv[idx]) for idx in range(len(hv) - int(nsga.agent.replace/nsga.NGEN) + 1, len(hv)+1)])
        
        mean_first = np.mean(first_hv, axis=0)
        std_first = np.std(first_hv, axis=0)
        mean_final = np.mean(final_hv, axis=0)
        std_final = np.std(final_hv, axis=0)
        
        if hv_no_agent != []:
            mean_no_agent = np.mean(hv_no_agent, axis=0)
            std_no_agent = np.std(hv_no_agent, axis=0)
        
        plt.figure()
        plt.title(f'Performance of agent: {name}')
        if errorbars:
            plt.errorbar(x=list(range(len(mean_first))), y=mean_first, yerr=std_first, capsize=5.0, label='First')
            plt.errorbar(x=list(range(len(mean_final))), y=mean_final, yerr=std_final, capsize=5.0, label='Last')
            if hv_no_agent != []:
                plt.errorbar(x=list(range(len(mean_no_agent))), y=mean_no_agent, yerr=std_no_agent, capsize=5.0, label='No Agent')
            
        else:
            plt.plot(list(range(len(mean_first))), mean_first, label='Random')
            plt.plot(list(range(len(mean_final))), mean_final, label='Trained')
            if hv_no_agent != []:
                plt.plot(list(range(len(mean_no_agent))), mean_no_agent, label='No Agent')
    ...
    hv = nsga.hv_dict
    a_file = open(f"HV_Agents_Run/hv_lr_1e-4_POPS_40.pkl", "wb")
    pickle.dump(hv, a_file)
    a_file.close()
    
    plot_first_last_hv(nsga=nsga, 
                       name=f'Agent_LR_1e-4_POP_40',
                       errorbars=False)
